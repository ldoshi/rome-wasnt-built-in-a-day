...
def has_gap_in_the_middle(states: list[np.ndarray]) -> list[np.ndarray]:
    ...
    for state in states:
        labels.append(np.sum(state.T[middle_one]) == 0 or np.sum(state.T[middle_two]) == 0)
    return labels
...
def has_small_gap_in_the_middle(states: list[np.ndarray]) -> list[np.ndarray]:
    ...
    for state in states:
        labels.append(
            (np.sum(state.T[middle_one]) == 0 or np.sum(state.T[middle_two]) == 0) and
            (np.sum(state.T[middle_one+1]) > 0) and
            (np.sum(state.T[middle_two-1]) > 0))
    return labels
