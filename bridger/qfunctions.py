...
class CNNQ(pl.LightningModule):
    def __init__(self, image_height, image_width, num_actions):
        ...
        args_iter = zip(
            channel_nums[:-1], channel_nums[1:], kernel_sizes, paddings, strides
        )
        ...
        for padding, kernel_size, stride in zip(paddings, kernel_sizes, strides):
            H = int((H + 2 * padding - kernel_size + 1) / stride) + 1
            W = int((W + 2 * padding - kernel_size + 1) / stride) + 1
        C = channel_nums[-1]
    ...
    def forward(self, x):
        x = x.reshape(-1, self.CNN[0].in_channels, self.image_height, self.image_width)
