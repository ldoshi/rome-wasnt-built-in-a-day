...
class TestOccurrenceLogger(unittest.TestCase):
    ...
    @parameterized.expand(
        [
            ...
            ("With LoggerAndNormalizer", list, str, [1,1], [2, 2], [2,1], [0,1,0,1,2,1]),
        ]
    )
    def test_occurrence_logging(
            self, name, log_entry_object_class, make_hashable_fn, object_0, object_1, object_2, expected_logged_objects
    ):
        ...
        with object_logging.ObjectLogManager(dirname=_TMP_DIR) as logger:
            ...
            self.assertEqual(occurrence_logger.get_top_n(), [object_1, object_0, object_2])
            self.assertEqual(occurrence_logger.get_top_n(2), [object_1, object_0])

        expected_entries = [log_entry.OccurrenceLogEntry(batch_idx=batch_idx, object=object) for batch_idx, object in zip([0,0,1,1,1,2], expected_logged_objects)]
    ...
    def test_logging_incorrect_type(self):
        with object_logging.ObjectLogManager(dirname=_TMP_DIR) as logger:
            ...
            self.assertRaises(ValueError, occurrence_logger.log_occurrence, batch_idx=0, object=[])

    def test_illegal_init_configuration(self):
        with object_logging.ObjectLogManager(dirname=_TMP_DIR) as logger:
            self.assertRaises(ValueError, object_logging.OccurrenceLogger, log_filename=_LOG_FILENAME_0,
                object_log_manager=logger,
                log_entry_object_class=torch.Tensor)
