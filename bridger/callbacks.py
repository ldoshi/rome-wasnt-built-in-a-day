...
class DemoCallback(Callback):
    def __init__(self, hparams, steps_per_update, max_episode_length):
        ...
        self._builder = builder.Builder(builder_trainer.make_env(hparams))
    ...
    def on_train_batch_end(
        ...
        if (batch_idx + 1) % self._frequency == 0:
            with torch.no_grad():
                build_result = self._builder.build(model.policy, self._max_episode_length)
                if build_result.is_done:
                    print(f"Built in {build_result.steps} steps with reward {build_result.reward}.")
